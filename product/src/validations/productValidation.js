...
async function validateProduct(product, validateCategoryId = true) {
  const slugRegex = new RegExp(/^[a-zA-Z 0-9\-]*$/);
  const errors = [];
  if (!validateName(product.name)) { errors.push('Product name must start with a letter and must contain more than 3 characters.'); }
