...
function validateAccount(account) {
    ...
    if(!validPasswordRegex.test(account.password))
        errors.push('The password must contain a letter, a number, a special character and must be at least 8 characters long.')
    if(!validCpfRegex.test(account.cpf))
        errors.push('Invalid CPF')
    if(!validPhoneRegex.test(account.phone))
...
function validateAddress(address) {
    ...
    if(!validCepRegex.test(address.cep))
        errors.push('Invalid CEP, CEP must contain exactly 8 digits')
    if(!ufs.includes(address.uf))
        errors.push('Invalid UF')
    return errors;
}
