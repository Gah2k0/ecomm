...
function validatePayment(payment) {
  ...
  const errors = [];
  if (isNullOrEmpty(payment.nameOnCard)) { errors.push('The provided name is invalid'); }
  if (payment.value <= 0) { errors.push('The payment value should be higher than 0'); }
  if (!cardNumberRegex.test(payment.cardNumber)) { errors.push('The provided card number is invalid'); }
