...
class PaymentsController {
  static createPayment = async (req, res) => {
    ...
    try {
      ...
      if (paymentValidationErrors.length > 0) return res.status(400).send({ message: paymentValidationErrors });
  ...
  static confirmPayment = async (req, res) => {
    ...
    try {
      ...
      if (payment.status !== PAYMENT_STATUS.CRIADO) { return res.status(400).send({ message: 'This payment status cannot be updated' }); }
  ...
  static cancelPayment = async (req, res) => {
    ...
    try {
      ...
      if (!payment || payment.status !== PAYMENT_STATUS.CRIADO) { return res.status(400).send({ message: 'This payment status cannot be updated' }); }
